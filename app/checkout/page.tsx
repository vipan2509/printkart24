...
export default function CheckoutPage() {
  ...
  const [whatsappHref, setWhatsappHref] = useState('https://wa.me/918288811860?text=Hi%20PRINTKART%2C%20I%20placed%20an%20order%20and%20would%20like%20to%20share%20the%20details.')
  ...
  async function placeOrder(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setWhatsappHref(`https://wa.me/918288811860?text=${encodeURIComponent(`Hi PRINTKART, I just placed an order.${orderId} Please confirm it.`)}`)
      clear()
      setDone(true)
    } catch {
  ...
  if (done) return <><Header /><main className="page-shell section-pad"><div className="success-state"><div className="success-icon"><Check /></div><h1>Thank you for your order.</h1><p>Your order has been received successfully. We&apos;ll send confirmation details to your email shortly.</p><div className="success-actions"><Link href="/" className="button-primary">Back to home</Link><a href={whatsappHref} target="_blank" rel="noreferrer" className="button-secondary">Notify us on WhatsApp</a></div></div></main><Footer /></>
