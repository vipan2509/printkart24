...
const categorySubcategories: Record<string, { label: string; image: string }[]> = {
  ...
  Stationery: [
    ...
    },
    {
      label: 'Stickers',
      image:
        'https://images.unsplash.com/photo-1608501078713-8e445a709b39?auto=format&fit=crop&w=400&q=80',
    },
    {
      label: 'Invitations',
      image:
...
function FilterButton({
  ...
  return (
    <div className="filter-group">
      <button type="button" className="catalog-filter" aria-expanded={active} onClick={onToggle}>
        {children}
        <ChevronDown size={16} />
      </button>
      {active && (
        <div className="filter-popover">
          {options.map((option) => (
            <button type="button" key={option} onClick={onToggle}>
              {option}
            </button>
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="marketplace-heading">
        <div>
          <h1>{title}</h1>
          <p>{products.length * 742 + 184} results</p>
        </div>
        <span className="marketplace-count">{products.length} featured products</span>
      </div>
      <div className="filter-bar">
        ...
        >
          <Grid2X2 size={20} /> Collections
        </button>
        <FilterButton
          options={['All categories', 'Best sellers', 'New arrivals']}
        ...
        <FilterButton
          options={['Cotton', 'Premium', 'Recycled']}
          active={activeFilter === 'material'}
        ...
        <FilterButton
          options={['Printed', 'Photo', 'Text']}
          active={activeFilter === 'decoration'}
          onToggle={() => toggleFilter('decoration')}
        >
          Decoration type
        </FilterButton>
        <button type="button" className="add-filter" onClick={() => toggleFilter('more')}>
          <SlidersHorizontal size={17} /> Add Filter <span>+</span>
        </button>
        <label className="sort-control">
          Sort by{' '}
          <select defaultValue="popular" aria-label="Sort products">
      ...
      <div className="marketplace-grid" id="products">
        <Link href="/contact" className="create-product-tile">
